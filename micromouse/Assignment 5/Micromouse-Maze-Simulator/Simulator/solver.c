...
Action floodFill(Mouse *mouse)
{
	...
	switch(curH) {
		case NORTH:
			if(x >= 1 && t.mDist[x-1][y] < curMDist && !t.knownHWalls[x][y]) {
				return FORWARD;
			}
			if(y < size-1 && t.mDist[x][y+1] < curMDist && !t.knownVWalls[x][y+1]) {
				return RIGHT;
			}
			if(y >= 1 && t.mDist[x][y-1] < curMDist && !t.knownVWalls[x][y]) {
				return LEFT;
			}
			break;
		case SOUTH:
			if(x < size-1 && t.mDist[x+1][y] < curMDist && !t.knownHWalls[x+1][y]) {
				return FORWARD;
			}
			if(y < size-1 && t.mDist[x][y+1] < curMDist && !t.knownVWalls[x][y+1]) {
				return LEFT;
	...
	while(!empty(&q)) {
		...
		// north cell
		if(x >= 1 && t.mDist[c.x-1][c.y] < min && !t.knownHWalls[c.x][c.y]) {
			min = t.mDist[c.x-1][c.y];
			printf("north of me is now min");
		}
		// south cell
		if(x < size-1 && t.mDist[c.x+1][c.y] < min && !t.knownHWalls[c.x+1][c.y]) {
			min = t.mDist[c.x+1][c.y];
			printf("south of me is now min");
		}
		// east cell
		if(y < size-1 && t.mDist[c.x][c.y+1] < min && !t.knownVWalls[c.x][c.y+1]) {
			min = t.mDist[c.x][c.y+1];
			printf("east of me is now min");

		}
		// west cell
		if(y >= 1 && t.mDist[c.x][c.y-1] < min && !t.knownVWalls[c.x][c.y]) {
			min = t.mDist[c.x][c.y-1];
			printf("west of me is now min");
		...
		if(t.mDist[c.x][c.y] <= min) {
			...
			t.mDist[c.x][c.y] = min + 1;;
			
			//while not wall, push the neighboring cells on to queue
			// north wall
			if(!t.knownHWalls[c.x][c.y]) {
				// add the north coordinate
				Coord add = {c.x-1, c.y};
				push(&q, add);
			}
			// south wall
			if(!t.knownHWalls[c.x+1][c.y]) {
				// add the south coordinate
				Coord add = {c.x+1, c.y};
				push(&q, add);
			}
			// east wall
			if(!t.knownVWalls[c.x][c.y+1]) {
				// add the east coordinate
				Coord add = {c.x, c.y+1};
				push(&q, add);
			}
			// west wall
			if(!t.knownVWalls[c.x][c.y]) {
				// add the west coordinate
				Coord add = {c.x, c.y-1};
				push(&q, add);
			}
		}
